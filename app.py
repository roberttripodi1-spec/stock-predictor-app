...
PAGES = ["dashboard", "details", "headlines", "share"]
PAGE_LABELS = {
    "dashboard": "Dashboard",
    "details": "Details",
    "headlines": "Headlines",
...
query_ticker = str(query_params.get("ticker", "AAPL")).strip().upper() if query_params.get("ticker") else "AAPL"
current_page = str(query_params.get("page", "dashboard")).strip().lower() if query_params.get("page") else "dashboard"
...
st.set_page_config(page_title="Stock Predictor", layout="wide", initial_sidebar_state="collapsed")
...
<style>
    ...
    @media (max-width: 768px) {
        .hero, .section-card, .movers-card { padding: 0.68rem 0.7rem; }
        .mover-link {
...
def draw_top_nav(active_page: str) -> str:
    nav_html = '<div class="section-card page-nav" style="padding:0.5rem 0.55rem;">'
    for page_key in PAGES:
        is_active = page_key == active_page
        bg = "#24324a" if is_active else "#162032"
        color = "#f8fafc" if is_active else "#d9e3f0"
        nav_html += (
            f'<a href="?ticker={st.session_state.active_ticker}&page={page_key}" '
            f'target="_self" style="display:inline-block;text-decoration:none;margin:.08rem .18rem .08rem 0;'
            f'padding:.45rem .72rem;border-radius:10px;background:{bg};color:{color};'
            'font-size:.8rem;font-weight:800;border:1px solid #223046;">'
            f'{PAGE_LABELS[page_key]}</a>'
        )
    nav_html += "</div>"
    return nav_html
...
with header_left:
    ...
    st.caption("Clean mobile-first stock dashboard.")
...
if not movers.empty:
    ...
    for _, row in movers.iterrows():
        ...
        mover_html += f'<a class="mover-link {css_class}" href="?ticker={ticker}&page={current_page}" target="_self">{label}</a>'
...
with st.sidebar:
    ...
    st.markdown("<div class='muted'>Use the native Streamlit menu button to open or close this panel.</div>", unsafe_allow_html=True)
...
should_run = run or st.session_state.auto_run or focus != query_ticker
...
st.markdown(draw_top_nav(current_page), unsafe_allow_html=True)

if error_message:
    st.error(f"Could not build the dashboard for {focus}: {error_message}")
    ...
    row1 = st.columns(4)
    row1[0].metric("Price", f"${safe_attr(result, 'latest_close', 0.0):,.2f}")
    row1[1].metric("Mood", safe_attr(result, "mood", "Neutral"))
    row1[2].metric("Up probability", f"{safe_attr(result, 'next_day_up_probability', 0.5):.2%}")
    row1[3].metric("Accuracy", f"{safe_attr(result, 'holdout_accuracy', 0.0):.2%}")

    row2 = st.columns(4)
    row2[0].metric("Momentum", f"{safe_attr(result, 'momentum_20d', 0.0):.2%}")
    row2[1].metric("Volume", f"{safe_attr(result, 'volume_ratio', 1.0):.2f}x")
    row2[2].metric("News tone", safe_attr(result, "sentiment_label", "Neutral"))
    row2[3].metric("Earnings", safe_attr(result, "earnings_flag", "No date found"))
    ...
    if current_page == "dashboard":
        ...
        sentiment_value = max(-1.0, min(1.0, safe_attr(result, "sentiment_score", 0.0)))
        g1, g2 = st.columns(2)
        with g1:
